...
| [data/eval_set.json](data/eval_set.json) | 21 道跨文档检索评测题 |
...
| GET | `/search?q=...&top_k=5&hybrid=true` | 混合检索最相关 chunk（`hybrid=false` 时仅向量），含来源与字符范围 |
| POST | `/ask` | RAG 问答，返回生成的回答与引用来源 |
| GET | `/eval/demo` | 内置演示评测集 |
| POST | `/eval/retrieval?mode=hybrid` | 检索评测：传入评测集，返回 Recall@K、MRR 与延迟（`mode=vector` 对比基线） |
