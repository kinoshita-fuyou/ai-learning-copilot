...
- **AI 应用**：RAG 全链路（文档解析、切分、Embedding、Top-K 检索、引用溯源）、
  工具调用 Agent、结构化输出、检索评测（Recall@K / MRR）
...
- 建立 Recall@K / MRR / 延迟评测体系，内置 20 道跨文档评测题，实测
  Recall@K=1.0、MRR=1.0、平均检索延迟约 0.4ms
...
- 20-question cross-document benchmark: Recall@K=1.0, MRR=1.0, ~0.4ms/search;
  26 tests green, GitHub Actions CI
